...
// fin suppValPoss
    ...
    public static void suppValPossExtension(int[][] gOrdi, boolean[][][] boolPossibles, int[][] nbValPoss) {
        int [][][]valPossibles = new int[9][9][2];
        int j1=0;
        int j2=0;
            ...
            for (int y1=0; y1<9; y1++){
                for (int x2=0; x2<9; x2++){
                    for (int y2=0; y2<9; y2++){
                        if (nbValPoss[x1][y1]==nbValPoss[x2][y2] && (x1!=x2 || y1!=y2)){
                            int[]case1={x1,y1};
                            int[]case2={x2,y2};
                            
                            for (int i = 1; i < 10; i++){
                                if (boolPossibles[case1[0]][case1[1]][i]==true){
                                    valPossibles[case1[0]][case1[1]][j1]=i;
                                    j1++;
                                }
                                if (boolPossibles[case2[0]][case2[1]][i]==true){
                                    valPossibles[case2[0]][case2[1]][j2]=i;
                                    j2++;
                                }
                            }
                            if(case1[0]==case2[0] && valPossibles[case1[0]][case1[1]] == valPossibles[case2[0]][case2[1]]){
                                for (int j = 0; j < 9; j++){
                                    if (gOrdi[case1[0]][j]==0 && j != case1[1] && j != case2[1]){
                                        if (boolPossibles[case1[0]][j][valPossibles[case1[0]][case1[1]][0]]==true){
                                            boolPossibles[case1[0]][j][valPossibles[case1[0]][case1[1]][0]]=false;
                                            nbValPoss[case1[0]][j]--;
                                        }
                                        if (boolPossibles[case1[0]][j][valPossibles[case1[0]][case1[1]][1]]==true){
                                            boolPossibles[case1[0]][j][valPossibles[case1[0]][case1[1]][1]]=false;
                                            nbValPoss[case1[0]][j]--;
                                        }
                                    }
                                }
                            }
                            if(case1[1]==case2[1] && valPossibles[case1[0]][case1[1]] == valPossibles[case2[0]][case2[1]]){
                                for (int i = 0; i < 9; i++){
                                    if (gOrdi[i][case1[1]]==0 && i != case1[0] && i != case2[0]){
                                        if (boolPossibles[i][case1[1]][valPossibles[case1[0]][case1[1]][0]]==true){
                                            boolPossibles[i][case1[1]][valPossibles[case1[0]][case1[1]][0]]=false;
                                            nbValPoss[i][case1[1]]--;
                                        }
                                        if (boolPossibles[i][case1[1]][valPossibles[case1[0]][case1[1]][1]]==true){
                                            boolPossibles[i][case1[1]][valPossibles[case1[0]][case1[1]][1]]=false;
                                            nbValPoss[i][case1[1]]--;
                                        }
                                    }
                                }
                            }
                            if(debCarre(3,case1[0],case1[1])==debCarre(3,case2[0],case2[1]) && valPossibles[case1[0]][case1[1]] == valPossibles[case2[0]][case2[1]]){
                                int[]debut=debCarre(3,case1[0],case1[1]);
                                for (int i = 0; i < 3; i++){
                                    for (int j = 0; j < 3; j++){
                                        int[]tab={i,j};
                                        if (gOrdi[debut[0]][debut[1]]==0 && tab != case1 && tab != case2){
                                            if (boolPossibles[debut[0]+i][debut[1]+j][valPossibles[case1[0]][case1[1]][0]]==true){
                                                boolPossibles[debut[0]+i][debut[1]+j][valPossibles[case1[0]][case1[1]][0]]=false;
                                                nbValPoss[debut[0]+i][debut[1]+j]--;
                                            }
                                            if (boolPossibles[debut[0]+i][debut[1]+j][valPossibles[case1[0]][case1[1]][1]]==true){
                                                boolPossibles[debut[0]+i][debut[1]+j][valPossibles[case1[0]][case1[1]][1]]=false;
                                                nbValPoss[debut[0]+i][debut[1]+j]--;
                                            }
                                        } 
                                    } 
 ...
 // fin initPossibles
    ...
    public static int tourHumain(int [][] gSecret, int [][] gHumain){
        ...
        while (trouve == false) {
            System.out.print("Coordonnée n°1(ligne): ");
            i = saisirEntierMinMax(1,9);
            System.out.print("Coordonnée n°2(colonne): ");
            j = saisirEntierMinMax(1,9);
            i --;
            j--;
            while (gHumain[i][j]!=0){
                System.out.print("Re Coordonnée n°1(ligne): ");
                i = saisirEntierMinMax(1,9);
                i--;
                System.out.print("Re Coordonnée n°2(colonne): ");
                j = saisirEntierMinMax(1,9);
                j--;
            }
            System.out.println("Quel chiffre voulez-vous saisir (Tapez 0 pour avoir un joker = 1 pénalité) ?");
