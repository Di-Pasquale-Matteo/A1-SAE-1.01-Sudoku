...
// fin suppValPoss
 ...
 // fin initPossibles
    ...
    public static int partie(){
        ...
        transformationGrilleSecrete(gSecret);
        int penaHumain = 0;
        int penaOrdi = 0;
 ...
 */
    ...
    public static void rotationGrille(int[][] grilleOriginale) {
        int n = grilleOriginale.length;
        int[][] grilleCopie = new int[n][n];
        copieMatrice(grilleOriginale, grilleCopie);
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                grilleOriginale[n-1-j][i] = grilleCopie[i][j];
            }
        }
    }
    
    public static void symetrieHorizontale(int[][] grille) {
        int n = grille.length;
        for (int i = 0; i < n / 2; i++){
            int[] temp = grille[i];
            grille[i] = grille[n-1-i];
            grille[n-1-i] = temp;
        }
    }

    public static void symetrieDiagonale(int[][] grille) {
        int n = grille.length;
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                int temp = grille[i][j];
    ...
    public static void echangeLignes(int[][] grille, int ligne1, int ligne2){
        if (ligne1 / 3 == ligne2 / 3) {
            for (int j = 0; j < grille[0].length; j++) {
                int temp = grille[ligne1][j];
                grille[ligne1][j] = grille[ligne2][j];
                grille[ligne2][j] = temp;
            }
        } 
    }
    ...
    public static void transformationGrilleSecrete(int[][] grille) {
        int nombreTransformations = Ut.randomMinMax(5, 15);
        for (int i = 0; i < nombreTransformations; i++) {
            int choix = Ut.randomMinMax(0, 3); 
            if (choix == 0) {
                rotationGrille(grille);
                System.out.println("Transformation appliquée: Rotation de 90 degrés");
            } else if (choix == 1) {
                symetrieHorizontale(grille);
                System.out.println("Transformation appliquée: Symétrie horizontale");
            } else if (choix == 2) {
                symetrieDiagonale(grille);
                System.out.println("Transformation appliquée: Symétrie diagonale");
            } else {
                
                int ligne1 = Ut.randomMinMax(0, 8);
                int ligne2 = Ut.randomMinMax(0, 8);
                while (ligne1 / 3 != ligne2 / 3 && ligne1 == ligne2) {
                    ligne1 = Ut.randomMinMax(0, 8);
                    ligne2 = Ut.randomMinMax(0, 8);
                }
                echangeLignes(grille, ligne1, ligne2);
                System.out.println("Transformation appliquée: Echange de ligne entre " + (ligne1+1) + " et " + (ligne2+1));
            }
        }
    }
