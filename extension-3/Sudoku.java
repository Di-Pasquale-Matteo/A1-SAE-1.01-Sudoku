...
public class Sudoku {
    ...
    public static void initGrilleComplete(int [][] gComplete){
        int[][] new_g = {{6,2,9,7,8,1,3,4,5},
                         ...
                         {9,5,8,3,1,2,4,7,6},
                         {7,3,2,4,5,6,1,8,9},
                         {1,6,4,8,7,9,2,5,3},public static void afficheGrille(int k, int[][] g) {
        int n = k * k;
        System.out.print("   ");
        for (int i = 1; i <= n; i++) {
            System.out.print(i + " ");
...
// fin suppValPoss
 ...
 // fin initPossibles
    ...
    /** pré-requis : ajout est un tableau de 2 nombre entier
     *  action :    ajout est ajouté à la fin de tabTrous 
     *              et tabTrous[0][0] augmente de 1 si tabTrous n'est pas remplit
     *  résultat :   renvoit faux si tabTrous est remplit et vrai sinon
     */
    public static boolean push(int[] ajout){
        if (tabTrous[0][0]==81){
            return false;
        }
        tabTrous[tabTrous[0][0]][0] = ajout[0];
        tabTrous[tabTrous[0][0]][1] = ajout[1];
        tabTrous[0][0]++;
        return true;
    }
    ...
    public static int[] pull(){
        ...
        int[] result = tabTrous[tabTrous[0][0]];
        tabTrous[tabTrous[0][0]][0] = 0;
        tabTrous[tabTrous[0][0]][1] = 0;
        tabTrous[0][0]--;
        return result;
    }
    ...
    public static int tourOrdinateur(int [][] gOrdi, boolean[][][] valPossibles, int [][]nbValPoss){
        ...
        System.out.println("L'ordi utilise un joker: quelle est la valeur dans la case" + trou[0]+1 + "," + trou[1]+1 + "?");
