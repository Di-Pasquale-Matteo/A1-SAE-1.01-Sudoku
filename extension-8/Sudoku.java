 ...
 // fin initPossibles
    ...
    public static int tourOrdinateur(int [][] gOrdi, boolean[][][] valPossibles, int [][]nbValPoss){
        ...
        if (nbValPoss[trou[0]][trou[1]] == 1) {
            for (int val = 1; val <= 9; val++) {
                if (valPossibles[trou[0]][trou[1]][val]) {
    ...
    public static int partie(){
        ...
        for (int i = 0; i < nbTrous; i++){
            ...
            methodeRefutation(gOrdi, valPossibles, nbValPoss);
            afficherPossibilites(gOrdi, valPossibles);
            penaOrdi = penaOrdi + tourOrdinateur(gOrdi,valPossibles,nbValPoss);
            System.out.println("Grille après le tour de l'ordinateur :");
 ...
 */
    ...
    public static boolean[][][] copierValPossibles(boolean[][][] valPossibles) {
        boolean[][][] copie = new boolean[9][9][10];
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                for (int k = 0; k < 10; k++) {
                    copie[i][j][k] = valPossibles[i][j][k];
                }
            }
        }
        return copie;
    ...
    public static int[][] copierNbValPoss(int[][] nbValPoss) {
        int[][] nbValPossCopie = new int[9][9];
        for (int i = 0; i < nbValPoss.length; i++) {
            for (int j = 0; j < nbValPoss[i].length; j++) {
                nbValPossCopie[i][j] = nbValPoss[i][j];
            }
        }
        return nbValPossCopie;
    } 

    public static boolean testerHypothese(int[][] gOrdi, boolean[][][] valPossibles, int[][] nbValPoss) {
        boolean[][][] valPossiblesCopie = copierValPossibles(valPossibles);
        int[][] nbValPossCopie = copierNbValPoss(nbValPoss);
    
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (gOrdi[i][j] != 0) {
                    suppValPoss(gOrdi, i, j, valPossiblesCopie, nbValPossCopie);
                }
            }
        }
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (gOrdi[i][j] == 0 && nbValPossCopie[i][j] == 0) {
                    return true; 
                }
            }
        }
    ...
    public static void methodeRefutation(int[][] gOrdi, boolean[][][] valPossibles, int[][] nbValPoss) {
        for (int i = 0; i < gOrdi.length; i++) {
            for (int j = 0; j < gOrdi[i].length; j++) {
                if (gOrdi[i][j] == 0) {
                    for (int val = 1; val <= 9; val++) {
                        if (valPossibles[i][j][val]) {
                            boolean[][][] valPossiblesCopie = copierValPossibles(valPossibles);
                            valPossiblesCopie[i][j] = new boolean[10]; 
                            valPossiblesCopie[i][j][val] = true; 
    
                            if (testerHypothese(gOrdi, valPossiblesCopie, nbValPoss)) {
                                valPossibles[i][j][val] = false;
                                nbValPoss[i][j]--;
                            }
                        }
                    }
    ...
    public static void afficherPossibilites(int[][] gOrdi, boolean[][][] valPossibles) {
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (gOrdi[i][j] == 0) {
                    System.out.print("Case [" + (i + 1) + "][" + (j + 1) + "]: ");
                    for (int k = 1; k <= 9; k++) {
                        if (valPossibles[i][j][k]) {
                            System.out.print(k + " ");
                        }
                    }
                    System.out.println();
